...
namespace Wojtas {
	...
	void Wojtas::wycinanie_rekurencyjne(std::string* slowo, std::string granica, bool strona)
	{

		if ((*slowo).find(granica) != std::string::npos)
		{
			if (strona)
			{
				size_t pozycja_obcinacza = (*slowo).find(granica);
				(*slowo).erase(0, pozycja_obcinacza + 1);
				wycinanie_rekurencyjne(slowo, granica, strona);
			}
			else 
			{
				size_t pozycja_obcinacza = (*slowo).find(granica);
				(*slowo).erase(pozycja_obcinacza, slowo->length());
				wycinanie_rekurencyjne(slowo, granica, strona);
			}
